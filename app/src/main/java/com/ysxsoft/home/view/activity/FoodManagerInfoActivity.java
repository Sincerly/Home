 ...
 **/
@Route(path = "/main/FoodManagerInfoActivity")
public class FoodManagerInfoActivity extends BaseActivity {
    ...
    public static void start(String did) {
        ARouter.getInstance().build(ARouterPath.getFoodManagerInfoActivity()).withString("did", did).navigation();
    }
    ...
    protected int getLayoutId() {
        return R.layout.activity_food_manager_info;
    }
...
//        }
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        if (resp != null) {
                            if (HttpResponse.SUCCESS.equals(resp.getCode())) {
                                ...
                                Glide.with(FoodManagerInfoActivity.this).load(d.getAvatar()).into(logo);
                                ...
                                Glide.with(FoodManagerInfoActivity.this).load(d.getPic()).into(shopHeaderImage);
                                Glide.with(FoodManagerInfoActivity.this).load(d.getCerficate()).into(shopQualificationImage);
                                Glide.with(FoodManagerInfoActivity.this).load(d.getCerficate()).into(shopQualificationImage2);

                                List<ShopManagerInfoResponse.DataBean.ImgBean> dataBeans = d.getImg();
                                datas.clear();
                                for (int i = 0; i < dataBeans.size(); i++) {
                                    Item item = new Item();
                                    item.setId(dataBeans.get(i).getImgid());
                                    item.setUrl(dataBeans.get(i).getUrl());
                                    datas.add(item);
                                }
                                initList();
                            } else {
    ...
    private void initList() {
        if (adapter == null) {
            adapter = new Adapter(R.layout.item_food_manager_info);
    ...
    private View createFooterView() {
        View view = View.inflate(this, R.layout.footer_food_manager, null);
        ...
        r.request(Manifest.permission.CAMERA).subscribe(new Consumer<Boolean>() {
            ...
            public void accept(Boolean aBoolean) throws Exception {
                if (aBoolean) {
                    ...
                    Intent photoPickerIntent = new BGAPhotoPickerActivity.IntentBuilder(FoodManagerInfoActivity.this)
    ...
    private class Adapter extends BaseQuickAdapter<Item, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder helper, Item item) {
            ImageView imgView = helper.getView(R.id.imgView);
            Glide.with(FoodManagerInfoActivity.this).load(item.getUrl()).into(imgView);
            ImageView pic = helper.getView(R.id.delete);
