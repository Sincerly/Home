 ...
 **/
@Route(path = "/main/FoodOrderbackDetailActivity")
public class FoodOrderbackDetailActivity extends BaseActivity {
    ...
    @Autowired
    boolean isShop;//是否是美食管理端查看

    public static void start(boolean isShop, String orderId) {
        ARouter.getInstance().build(ARouterPath.getFoodOrderbackDetailActivity()).withBoolean("isShop", isShop).withString("orderId", orderId).navigation();
    }
...
//        initProductList();
    ...
    protected int getLayoutId() {
        return R.layout.activity_food_orderback_detail;
    }
    ...
    private void initList(List<String> pic) {
        ...
        if (!pic.isEmpty()) {
            ...
            GridLayoutManager manager = new GridLayoutManager(FoodOrderbackDetailActivity.this, spanCount);
            manager.setOrientation(GridLayoutManager.VERTICAL);
            recyclerView.setLayoutManager(manager);
            recyclerView.setAdapter(new RBaseAdapter<String>(FoodOrderbackDetailActivity.this, R.layout.item_image, pic) {

                @Override
                protected void fillItem(RViewHolder holder, String item, int position) {
                    ImageView image = holder.getView(R.id.image);
                    Glide.with(FoodOrderbackDetailActivity.this).load(item).into(image);
                }
    ...
    private void initProductList(List<FoodBackDetailResponse.DataBean.ProductBean> product) {
        ...
        if (!product.isEmpty()) {
            productRecyclerView.setLayoutManager(new LinearLayoutManager(FoodOrderbackDetailActivity.this));
            productRecyclerView.setAdapter(new RBaseAdapter<FoodBackDetailResponse.DataBean.ProductBean>(FoodOrderbackDetailActivity.this, R.layout.item_food_order_child_product, product) {
                ...
                protected void fillItem(RViewHolder holder, FoodBackDetailResponse.DataBean.ProductBean item, int position) {
                    ...
                    Glide.with(FoodOrderbackDetailActivity.this).load(item.getPic()).into(pic);
                    ...
                    holder.setText(R.id.rule, item.getRule_name());
                }
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        if (resp != null) {
                            if (HttpResponse.SUCCESS.equals(resp.getCode())) {
                                FoodBackDetailResponse.DataBean s = resp.getData();
                                //初始化状态
                                //初始化状态
...
//                                    0没申请退货，1退货退款，2仅退款
                                    ...
                                    if (needShowTui) {
                                        ...
                                        left.setOnClickListener(new View.OnClickListener() {
                                            ...
                                            public void onClick(View v) {
                                                //拒绝退款
                                                ShopManagerSubmitMessageActivity.start(FoodOrderbackDetailActivity.this, 0x02, false, orderId);
                                            }
                                        });
                                ...
                                baojine.setText(s.getBao());

                                if(isShop){
                                    baojine.setVisibility(View.VISIBLE);
                                }
                                ...
                                //初始化商品
                                initProductList(s.getProduct());
                                totalProduct.setText("共" + s.getProduct().size() + "件商品 总计：");
                                totalMoney.setText("￥" + s.getAccount());
                            }
                        } else {
