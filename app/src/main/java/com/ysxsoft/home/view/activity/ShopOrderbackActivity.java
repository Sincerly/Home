 ...
 **/
@Route(path = "/main/ShopOrderbackActivity")
public class ShopOrderbackActivity extends BaseActivity implements BGASortableNinePhotoLayout.Delegate {
    ...
    public static void start(boolean isShop, String orderId, int type) {
        ARouter.getInstance().build(ARouterPath.getShopOrderbackActivity()).withInt("type", type).withBoolean("isShop", isShop).withString("orderId", orderId).navigation();
    }

    public static void start(Activity activity,int requestCode,boolean isShop, String orderId, int type) {
        ARouter.getInstance().build(ARouterPath.getShopOrderbackActivity()).withInt("type", type).withBoolean("isShop", isShop).withString("orderId", orderId).navigation(activity,requestCode);
    }
...
//        initProduct();
    ...
    private void initProduct(List<ShopOrderDetailResponse.DataBean.ProductBean> products) {
        //填充商品
        productRecyclerView.setNestedScrollingEnabled(false);
        if (!products.isEmpty()) {
            productRecyclerView.setLayoutManager(new LinearLayoutManager(this));
            productRecyclerView.setAdapter(new RBaseAdapter<ShopOrderDetailResponse.DataBean.ProductBean>(this, R.layout.item_shop_order_child_product, products) {
                ...
                protected void fillItem(RViewHolder holder, ShopOrderDetailResponse.DataBean.ProductBean item, int position) {
                    ...
                    Glide.with(ShopOrderbackActivity.this).load(item.getPic()).into(pic);
                ...
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        if (resp != null) {
                            if (HttpResponse.SUCCESS.equals(resp.getCode())) {
                                //请求成功
                                EventBus.getDefault().post("", "refreshShopOrderList");//刷新家美食订单列表
                                setResult(RESULT_OK);
                                finish();
                            } else {
                                //请求失败
